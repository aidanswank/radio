...
io.on('connection', function (socket) {
	...
	// console.log(socket.id, " ", socketToNickname[socket.id], socketToNickname, socket.handshake.session.nickname)
	...
	socket.on('set nickname', function (nickname) {
		...
		io.emit('connected users', Object.values(sessionToNickname));
		// io.emit('connected users', Object.values(socketToNickname));
	});
	...
	socket.on('chat message', function (msg) {
		...
		if (!nickname || nickname.trim() === '') {
		// Handle the case where the nickname is not set
		...
		if (!msg || msg.trim() === '') {
		// Handle empty message here, maybe notify the user or do nothing
